...
require_once '_ocxpak/conf.php';
require_once '_ocxpak/func.php';
require_once '_ocxpak/reqd.php';
...
if (isset($clo[MAKEZIP]) && $clo[MAKEZIP] !== false) {
	require_once '_ocxpak.def.php';

	$workdir = getWd($clo[MAKEZIP]);
	if ($workdir) {
		$ocvers = $clo[OCVERS];
		$srcdir = getConcatPath($workdir, SRCDIR, $ocvers);
		...
		$zipfile = getConcatPath($zipdir, MODFILE . '-oc' . $ocvers . ZIPEXT);
